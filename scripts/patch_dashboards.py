...
<style>
  ...
  /* 100dvh, not 100vh: on iOS Safari 100vh is the LARGE viewport, so a pinned
     map runs under the browser toolbar and takes its controls with it. The
     vh line stays first as the fallback. iPhone is the ONLY path here -
     Safari exposes no requestFullscreen on a div, so this pinned mode is
     what every iPhone visitor gets. */
  .leaflet-container.rs-maxi{position:fixed!important;inset:0!important;width:100vw!important;height:100vh!important;height:100dvh!important;z-index:2000!important;border-radius:0}
...
window.addEventListener('load', function () {
  ...
  var t = setInterval(function () {
    ...
    var setMaxi = function (on) {
      el.classList.toggle('rs-maxi', on);
      // without this the document behind keeps scrolling, and its scrollbars
      // sit over the map
