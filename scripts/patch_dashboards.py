...
window.addEventListener('load', function () {
  ...
  var t = setInterval(function () {
    ...
    // Leaflet's window-resize handler repositions the pane mid-gesture on
    // machines whose full-screen transition resizes in steps; the drag handler
    // then teleports back to its stale cached position. Single owner: settle.
    if (map._onResize) {
      L.DomEvent.off(window, 'resize', map._onResize, map);
      L.DomEvent.off(window, 'resize', map._onResize);
      map.options.trackResize = false;
    }
    var held = false, rearm = false;
    ...
    var settle = function () {
      ...
      settleTimer = setTimeout(function () {
        if (held || (map.dragging && map.dragging.moving())) {
          rearm = true;               // finish after the hand lifts
          return;
        }
